...
import { digitalizationArticle } from "./articles/DigitalizationArticle";
...
const addCommonCTA = (content: string) => {
  const ctaHtml = `
    <div class="bg-[rgba(0,0,46,0.05)] p-8 rounded-xl mb-12">
      ...
      <a href="/contatto" class="inline-block bg-[rgba(0,0,46,255)] text-white px-6 py-3 rounded-lg hover:bg-[rgba(0,0,46,0.9)] transition-colors">Prenota una chiamata gratuita</a>
    </div>
  `;
...
export const blogPosts = {
  ...
  "digitalizzazione-sicurezza-lavoro-tecnologie": {
    ...digitalizationArticle,
    content: addCommonCTA(digitalizationArticle.content)
  }
};